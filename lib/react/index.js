...
var Bullet = function (_Component) {
    ...
    _createClass(Bullet, [{
        ...
        value: function run() {
            ...
            bullets = bullets.map(function (item) {
                ...
                if (linesArray.length > 0) {
                    ...
                    item.position = {
                        x: _this2.canvasInfo.canvas.width,
                        y: (lineNum - 0.5) * _this2.canvasInfo.bullet.lineHeight + (_this2.canvasInfo.bullet.lineHeight - _this2.canvasInfo.bullet.borderHeight) //最上一条的弹幕距离视频顶部
                    };
                    item.line = lineNum;
                    item.run = true;
                    item.width = _this2.getBulletWidth(_this2.canvasInfo.bullet.fontSize, item.text, !!item.logo); //获取当前弹幕的宽度
                    return item;
                } else {
                    return item;
                }
            ...
            bullets.map(function (item) {
                if (item.run) {
                    item.position.x -= _this2.canvasInfo.canvas.lineSpeed[item.line - 1]; //移动弹幕
                    if (item.position.x + item.width + 20 > 0) {
                        if (item.position.x + item.width + 20 < 300 && !item.release) {
                            //释放当前行数
                            _this2.canvasInfo.canvas.linesArray.push(item.line);
                            item.release = true;
                        }
            ...
            this.loop = requestAnimationFrame(function () {
                if (_this2.add.length > 0) {
                    _this2.bullets = _this2.bullets.concat(_this2.addBullets); //新添加的弹幕
                    _this2.add = [];
                }
                _this2.run();
            });
        ...
        value: function render() {
            ...
            return _react2.default.createElement('canvas', { style: { position: "absolute", left: "0", top: "0" }, id: 'myCanvas', ref: function ref(_ref) {
                    _this3.canvas = _ref;
